...
const TString geoVersion = "ATTPCSimulationv1.1";
...
const Float_t Altube_ID = 2.3622;
const Float_t Altube_length = 7.62;
...
TGeoVolume* create_Altube() //creating aluminum insert for the G10 tube
{
  ...
  gGeoMan->GetVolume(geoVersion)->AddNode(Altube_volume,1, new TGeoTranslation(0,0,Altube_length/2+22.4));	//center the aluminum tube against the back wall and shift it along z axis forward 22.4 cm
...
TGeoVolume* create_G10Vacuum()	//creating a vacuum to be "inserted" into the G10 pipe
{
  ...
  TGeoVolume *vacuum_volume = gGeoManager->MakeTube("G10vacuum_volume", TubeVacuum,0, tube_ID/2, 22.4/2); //third parameter is the ID
  //TGeoVolume *vacuum_volume = gGeoManager->MakeBox("vacuum_volume", gas,  100./2, 100./2, 100./2);
  gGeoMan->GetVolume(geoVersion)->AddNode(vacuum_volume,1, new TGeoTranslation(0,0,22.4/2));	//USE THIS TO TRANSLATE AND MOVE TUBES
...
TGeoVolume* create_AlTubeCap()	//creating end cap for pipe
{
  ...
  TGeoVolume *Cap_volume = gGeoManager->MakeTube("Cap_volume", TubeCap,0, Altube_OD/2, 0.04/2); //third parameter is the ID
  //TGeoVolume *Cap_volume = gGeoManager->MakeBox("Cap_volume", gas,  100./2, 100./2, 100./2);
  gGeoMan->GetVolume(geoVersion)->AddNode(Cap_volume,1, new TGeoTranslation(0,0,0.04/2+22.4+Altube_length));	//center the cap to datum and shift it forward the same distance as alum tube + length of tube
...
TGeoVolume* create_LithiumEvp()	//creating lithium target
{
  ...
  gGeoMan->GetVolume(geoVersion)->AddNode(Li_volume,1, new TGeoTranslation(0,0,0.000498/2+22.4+Altube_length-0.000498));	//center the target to datum and shift it down the g10 pipe, then the aluminum pipe, but bring in slightly due to target thickness...cannot hang out the back
...
TGeoVolume* create_AlVacuum()	//creating a vacuum to be "inserted" into the Aluminum pipe
{
  ...
  gGeoMan->GetVolume(geoVersion)->AddNode(Alvacuum_volume,1, new TGeoTranslation(0,0,Altube_length/2+22.4));	
